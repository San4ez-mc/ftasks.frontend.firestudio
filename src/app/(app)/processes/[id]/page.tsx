...
import { ArrowLeft, Plus, Save, X, GripVertical, AlertTriangle, Lightbulb, CircleHelp, Database, Loader2 } from 'lucide-react';
...
import { getProcess, updateProcess } from '@/app/(app)/processes/actions';
import { getOrgStructureData } from '@/app/(app)/org-structure/actions';
...
export default function Page({ params }: ProcessesPageProps) {
  ...
  const [allSections, setAllSections] = useState<(Section & { departmentName: string })[]>([]);
  const [allEmployees, setAllEmployees] = useState<Employee[]>([]);

  useEffect(() => {
    startTransition(async () => {
      try {
        const [processData, orgData] = await Promise.all([
          getProcess(id),
          getOrgStructureData()
        ]);
        
        if (!processData) {
          toast({ title: "Помилка", description: "Процес не знайдено.", variant: "destructive" });
        ...
        const sectionsWithDept = orgData.departments.flatMap(dept =>
          (dept.sections || []).map(sec => ({
            ...sec,
            departmentName: dept.name
          }))
        );
        setAllSections(sectionsWithDept);

        setAllEmployees(orgData.employees);
        setUsers(orgData.employees.map(e => ({ id: e.id, name: e.name, avatar: e.avatar || '' })));

      } catch (error) {
        toast({ title: "Помилка завантаження", description: "Не вдалося завантажити дані для редактора.", variant: "destructive" });
      }
    });
  }, [id, router, toast]);

  if (isPending || !process) {
    return (
  ...
  return (
    <ProcessEditor
      key={process.id} // Re-mount component if process changes
...
function ProcessEditor({ initialProcess, users, allSections, allEmployees }: ProcessEditorProps) {
  const router = useRouter();
