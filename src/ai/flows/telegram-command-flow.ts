...
const CreateResultInputSchema = z.object({
      ...
      subResults: z.any().optional().describe("Масив вкладених підрезультатів. Модель повинна сама розпізнати вкладену структуру з тексту."),
...
export async function parseTelegramCommand(input: TelegramCommandInput): Promise<TelegramCommandOutput> {
  ...
  const systemPrompt = `Ти — агент-парсер команд для системи керування задачами Fineko.

**Твоя єдина мета:** перетворити текст користувача на виклик одного з доступних інструментів.

**СУВОРІ ПРАВИЛА:**
1.  **ЗАВЖДИ ВИКЛИКАЙ ІНСТРУМЕНТ:** Проаналізуй запит і виклич найбільш відповідний інструмент.
2.  **УТОЧНЮЙ, ЯКЩО НЕ ВИСТАЧАЄ ДАНИХ:** Якщо для виклику інструмента не вистачає обов'язкових параметрів (наприклад, команда "створи задачу" без назви), **ЗАВЖДИ** використовуй інструмент \`clarify\`.
3.  **НЕ ВІДПОВІДАЙ НАПРЯМУ:** Ніколи не відповідай користувачу розмовним текстом. Якщо запит "покажи мої задачі", ти **ЗОБОВ'ЯЗАНИЙ** викликати інструмент \`view_tasks\`.
4.  **ПІСЛЯ ІНСТРУМЕНТУ — ЗАВЕРШУЙ:** Коли інструмент буде викликаний і ти отримаєш його результат, твоя фінальна відповідь має бути ОДНЕ СЛОВО: \`OK\`. Більше нічого не генеруй. Це сигнал про завершення роботи.

**КОНТЕКСТ:**
- Сьогоднішня дата: ${new Date().toISOString().split('T')[0]}. Використовуй її для розрахунку дат ("сьогодні", "завтра").
- Списки співробітників та шаблонів надані для точного заповнення параметрів. Імена мають збігатися.`;
