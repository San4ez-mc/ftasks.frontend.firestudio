...
- **Обробка команд природною мовою:** Бот використовує AI для розпізнавання команд, написаних звичайною мовою (включаючи аудіоповідомлення).
  - **Статус:** 🟡 Частково реалізовано (Логіка перенесена на зовнішній бекенд).
...
- **Процес автентифікації:**
    1.  Користувач натискає кнопку, що перенаправляє його до Telegram-бота.
    2.  Бот (через зовнішній бекенд) надсилає користувачу повідомлення з кнопкою "Завершити вхід".
    3.  Ця кнопка містить URL для повернення в додаток (`/auth/telegram/callback`) з тимчасовим токеном.
    4.  Додаток отримує токен, валідує його через зовнішній бекенд і, залежно від наявності компаній у користувача, перенаправляє його на головну сторінку, сторінку вибору компанії або створення нової.
  - **Статус:** ✅ Реалізовано.
...
Опис функціоналу, який виконує Telegram-бот на **зовнішньому бекенді**. Детальна специфікація для розробки цієї логіки знаходиться у файлі [`docs/telegram-bot-logic.md`](./telegram-bot-logic.md).

- **Автентифікація:** Описано в розділі 9.
  - **Статус:** 🟡 Перенесено на зовнішній бекенд.
...
- **Обробка команд:**
    ...
    2.  Отриманий текст (або транскрибований текст з аудіо) відправляється в AI-flow для розпізнавання команди.
    3.  Серверна логіка виконує цю команду (створює задачу, показує список співробітників і т.д.) і надсилає відповідь у чат.
  - **Статус:** 🟡 Перенесено на зовнішній бекенд.
```